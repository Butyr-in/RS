...
const App = {
    ...
    updateTree(hands) {
        ...
        dayGroups.forEach((day) => {
            ...
            // --- Период дня для отображения (как у сессий) ---
const firstHandOfDay = day.hands[0];
const lastHandOfDay = day.hands[day.hands.length - 1];
const dayStartTime = firstHandOfDay.startTime.substring(0, 5);
const dayEndTime = lastHandOfDay.startTime.substring(0, 5);
const dayDateShort = `${startParts[2]}.${startParts[1]}`;
...
dayHeader.innerHTML = `
    <div class="day-info">
        <span class="day-date">📅 ${dayDateShort} ${dayStartTime} — ${dayEndTime}</span>
        <span class="pill avg-limit" data-copy="${avgLimitCopy}">NL${avgLimitDisplay}</span>
        <span class="pill hands" data-copy="${day.hands.length}">${day.hands.length}</span>
        <span class="pill duration" data-copy="${dayDurationMinutes}">${dayDurationMinutes} мин</span>
        <span class="day-profit ${profitClass}">${Stats.formatMoney(dayProfit, currency)}</span>
    </div>
...
sessionItem.innerHTML = `
    <div class="session-info">
        <span class="session-time">⏱ ${sessionShortDate} ${timeStart} — ${timeEnd}</span>
        ...
        <span class="session-profit ${sProfitClass}">${Stats.formatMoney(sessionProfit, currency)}</span>
    </div>
`;
